...
function getVersion() { return "14.11c"; }
...
function getImageO(imgbase, noi, baseno, per, alpha, next)
{
	...
	if (ar.length > 1) {
		...
		else if (st == "Player" || per === perYou) {
			obj = obj.Player[ar[1]];
			if (obj === undefined) obj = oImages;
		} 
		img = ar[ar.length - 1].trim();
	}
	if (st == "People" || per !== undefined) {
		if (per === perYou) {
	...
	// Is this an explicit image (base on the image passed in NOT isExplicit()
	var bExplicit = false;
...
function onerrorImage(el)
{
	...
	if (el.getAttribute("data-img") === null) el.setAttribute('data-img', img);
	/*
	...
	if (vidn.id === "viderr") {
		...
		for (var i = 0; i < ppn.childNodes.length; i++) {
			...
			if (curChild.nodeType == Node.ELEMENT_NODE && curChild.tagName.toLowerCase() === "img") {
				...
				var imgo = curChild.getAttribute('data-img');
				//console.log('imgo = ' + imgo);
				if (imgv.indexOf("Mods/") != -1 || imgv.indexOf("Explicit/") != -1) {
					if (imgv.indexOf("Explicit/") != -1) {
						// Fallback from Explicit to non
						imgv = imgv.split("Explicit/").join("");
					} else {
						// Fallback from Mods to base Images/, based on the original filename (to allow fallback again from Explicit)
						imgv = imgo.split("Mods/" + gameState.sMod + "/").join("");
					}
					var ar = imgo.split('.');
					var ext = ar[ar.length - 1].split('#')[0];	// Extension of the image
					if (ext === null) ext = "jpg";
