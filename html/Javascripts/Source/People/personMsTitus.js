...
{
	// Ms Titus
	addPerson("Ms. Titus", 3, "MsTitus", 'Natural', false);
	...
	per.showEvent = function()
	{
		...
		if (sType == "titsfuck") {
			...
			addPlaceTitle(md, "Playing with Ms. Titus");
			md.write('<p>' + herNameT + '\'s large breasts are difficult to ignore, so you ask her to use her breasts to pleasure you. This seems to be something she is <i>very</i> familiar with and she uses her very large bussom to please you until you release all over her breasts and face.</p>');
			startQuestions();
			addLinkToPlace(md, sType == "monique" ? 'tell them that is enough for now' : 'tell Ms. Titus she\'s had enough for now', 3);
			WritePlaceFooter(md);
			return true;
	...
	per.handleItem = function(no, cmd)
	{
		...
		// Casting the transform spell
			...
			if ((this.isHere() && sType === "")) {
				...
				// It can be cast
				//ClearComments();
				dispPlace(Place, 'type=mstitustransformage' + this.getDress().toLowerCase().split("/").join(""));
				return "handled";
			}
		}
