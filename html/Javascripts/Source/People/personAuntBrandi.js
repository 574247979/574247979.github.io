...
function initialiseAuntBrandi()
{
	...
	per.showEventLounge = function()
	{
		...
		if (sType == "branditflounge") {
			...
			md.write('<p>Kylie watches her mother and you with some arousal, and when Aunt Brandi notices she gives some pointers for in the future, though she does comment how Kylie is not quite ready yet, a reference to Kylies relatively small breasts.</p>');
		...
		if (sType == "brandikylielounge") {
			...
			if (this.isNympho()) {
				md.write(
					'<p>You ask Kylie and Aunt Brandi put on a show for you and they both enthusiastically embrace each other. You see no reservations at this incestuous act, then again what you have done with them is borderline, incestouus as well in some jurisdictions.</p>' +
	...
	per.showPersonChat = function(bGeneral, md)
	{
		if (Place == 400 && this.isHere() && sType === "") {
			...
			if (this.isCharmedBy()) {
				...
				if (this.isNympho()) {
					...
					if (perYou.isMaleSex()) addLinkToPlaceC(md, "fuck your slave\'s tits", Place, 'type=brandirflounge');
		...
		if (Place == 402 && this.isHere() && sType === "") {
			...
			if (this.isNympho()) {
				...
				if (perYou.isMaleSex()) addLinkToPlaceC(md, "fuck your slave\'s tits", Place, 'type=brandirfbed');
			}
